...
class Config extends LogicBase
{
    ...
    public function getConfigListFilter($param = [])
    {
        ...
        !empty($param['keyword']) && $where['name|title'] = ['like', '%'.(string)$param['search_data'].'%'];
		...
		$limit = !empty($param['limit'])?$param['limit']:DB_LIST_ROWS;
        
        $data['list'] = $this->getConfigList($where, true, $sort,$limit);
    ...
    public function configAdd($data = [])
    {
        ...
        if (!$validate_result) {
			...
			return setRetrunData($this->validateConfig->getError(),'',1);
        }
		...
		//[RESULT_SUCCESS, '配置添加成功', $url] : [RESULT_ERROR, $this->modelConfig->getError()];
    }
